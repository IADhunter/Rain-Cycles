...
public static class CustomModeState
{
    ...
    public static void Activate(string regionCode, string triggerId)
    {
        ...
        {
            RSPlugin.log.LogWarning("[CustomMode] Activate: regionCode y triggerId no pueden ser vacíos.");
            return;
        }
        ...
        RSPlugin.log.LogInfo($"[CustomMode] Activated '{triggerId}' for region '{key}'.");
    ...
    public static void Deactivate(string regionCode, string triggerId)
    {
        ...
        RSPlugin.log.LogInfo($"[CustomMode] Deactivated '{triggerId}' for region '{key}'.");
    ...
    private static void Load()
    {
        ...
        {
            RSPlugin.log.LogWarning($"[CustomMode] Error loading state: {ex.Message}");
        }
    }
    ...
    private static void Save()
    {
        ...
        {
            RSPlugin.log.LogWarning($"[CustomMode] Error saving state: {ex.Message}");
        }
    }
    ...
    private static void TryStartClockForCurrentRegion(string regionCode, string triggerId)
    {
        ...
        if (settings._hasRoomsSection)
            ...
            {
                int count = StateFileResolver.CountRainStateFiles(room);
